...
- The main task owns integration and alignment. The latest user decision is: frontend and independent QA tasks use GPT-6 Luna max; only the backend task uses GPT-6 Astra low. The main task owns design, code review, integration and real UI acceptance. Do not override this with older model settings.
...
Keep task prompts outcome-focused: context, ownership, expected behavior and relevant evidence. Let each assigned task choose implementation steps within its ownership. Use plain Chinese to report what changed, what was actually checked and what remains. A build, mock model or screenshot alone is not proof of an end-to-end feature.
