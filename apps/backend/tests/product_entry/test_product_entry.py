...
class ImportNegativeTests(_CleanStorageCase):
    def test_tampered_payload_byte_fails_and_registers_nothing(self) -> None:
        ...
        spec_file = next(item for item in files if item["path"] == "spec.json")
    ...
    def test_declared_hash_lie_fails(self) -> None:
        files = copy.deepcopy(self.files)
        target = next(item for item in files if item["path"] == "adjusted.json")
        target["sha256"] = "0" * 64
        with self.assertRaises(InvalidArgumentError):
    ...
    def test_path_traversal_and_unknown_file_names_fail(self) -> None:
        ...
        files2[0] = {**files2[0], "path": "sub/dir/file.json"}
        with self.assertRaises(InvalidArgumentError):
    ...
    def test_missing_required_file_fails(self) -> None:
        files = [item for item in self.files if item["path"] != "policy.json"]
        with self.assertRaises(InvalidArgumentError):
    ...
    def test_caller_numeric_arrays_are_absent_from_request_surface(self) -> None:
        ...
        for item in self.files:
            self.assertEqual(
                set(item), {"path", "sha256", "byte_size", "payload_base64"}
            )
