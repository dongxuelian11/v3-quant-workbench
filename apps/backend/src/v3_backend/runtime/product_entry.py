...
_DESCRIPTOR_KEYS = {"path", "artifact_id", "sha256", "byte_size"}
...
def _parse_descriptor(value: Any, name: str) -> dict[str, Any]:
    ...
    path = _require_package_path(value["path"], f"{name}.path")
...
def parse_package_manifest(wire: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for item in artifacts:
        if not isinstance(item, Mapping) or set(item) != {"row", "path"}:
        ...
        if byte_size > 0:
            path = _require_package_path(item["path"], "artifacts.path")
        else:
            ...
            path = None
        parsed_artifacts.append({"row": row, "path": path})
...
def _decode_files(files_wire: Any) -> dict[str, PackageFile]:
    ...
    for item in files_wire:
        if not isinstance(item, Mapping) or set(item) != {"path", "sha256", "byte_size", "payload_base64"}:
            raise InvalidArgumentError("each package file must be a closed {path, sha256, byte_size, payload_base64}")
        path = _require_package_path(item["path"], "files.path")
        if path in decoded:
...
def import_research_package(
    ...
    *,
    ...
    spec_file = files[manifest["run_spec_artifact"]["path"]]
    context_file = files[manifest["execution_context_artifact"]["path"]]
    ...
    for table in PACKAGE_OWNER_TABLES:
        ...
        payload_file = files[descriptor["path"]]
...
def _verify_closed_file_set(manifest: Mapping[str, Any], files: Mapping[str, PackageFile]) -> None:
    expected: set[str] = {
        manifest["run_spec_artifact"]["path"],
        manifest["execution_context_artifact"]["path"],
    }
    for entry in manifest["artifacts"]:
        if entry["path"] is not None:
            expected.add(str(entry["path"]))
...
def build_research_package(
    ...
    try:
        ...
        for table in PACKAGE_OWNER_TABLES:
            ...
            artifact_rows.append({"row": artifact, "path": path})
        ...
        artifact_rows.append({"row": spec_artifact, "path": "spec.json"})
        ...
        artifact_rows.append({"row": context_artifact, "path": "context.json"})
        ...
        for carried in carried_rows:
            for column, value in carried.items():
                ...
                if int(reached["byte_size"]) > 0:
                    ...
                    artifact_rows.append({"row": reached, "path": reached_path})
                    payload_plans.append((reached_path, reached_id))
                else:
                    paths[reached_id] = ""
                    artifact_rows.append({"row": reached, "path": None})
