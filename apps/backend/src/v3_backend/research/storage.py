...
class Store:
    ...
    def experiments(self, project_id):
        return [{**item, **({'strategyId': 'default'} if project_id and 'strategyId' not in item else {})} for item in self.project_store(project_id).list('experiment', project_id or '')]

    def experiment(self, project_id, experiment_id):
        value = self.project_store(project_id).get('experiment', experiment_id)
        if project_id and 'strategyId' not in value:
