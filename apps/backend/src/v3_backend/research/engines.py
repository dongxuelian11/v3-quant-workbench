...
def validate_expression(expression):
    ...
    validate_price_basis(expression)
    return expression


def validate_price_basis(expression):
    """Custom factors cannot depend on a later forward-adjustment rescaling."""
    tree=ast.parse(re.sub(r'\${1,2}([A-Za-z_][A-Za-z0-9_]*)',r'field_\1',expression),mode='eval')
    adjusted={'field_open','field_high','field_low','field_close','field_vwap','field_factor'}
    inverse={'field_volume','field_knownvolume'}
    if not any(isinstance(n,ast.Name) and n.id in adjusted|inverse for n in ast.walk(tree)):return
    def degree(n):
        if isinstance(n,ast.Expression):return degree(n.body)
        if isinstance(n,ast.Constant):return 0
        if isinstance(n,ast.Name):return 1 if n.id in adjusted else -1 if n.id in inverse else 0
        if isinstance(n,ast.UnaryOp):return degree(n.operand)
        if isinstance(n,ast.BinOp):
            a,b=degree(n.left),degree(n.right)
            if a is None or b is None:return None
            if isinstance(n.op,(ast.Add,ast.Sub)):return a if a==b else None
            if isinstance(n.op,ast.Mult):return a+b
            if isinstance(n.op,ast.Div):return a-b
            if isinstance(n.op,ast.Pow) and isinstance(n.right,ast.Constant):return a*n.right.value
        if isinstance(n,ast.Compare):
            values=[degree(n.left),*[degree(v) for v in n.comparators]]
            return 0 if None not in values and len(set(values))==1 else None
        if isinstance(n,ast.Call):
            name=n.func.id;args=[degree(v) for v in n.args]
            if any(v is None for v in args):return None
            if name in {'Rank','Sign','Rsquare','Corr'}:return 0
            if name=='Cov':return args[0]+args[1]
            if name=='Log':return 0 if args[0]==0 else None
            if name=='Power':return args[0]*n.args[1].value if isinstance(n.args[1],ast.Constant) else 0 if args[0]==args[1]==0 else None
            if name=='If':return args[1] if args[0]==0 and args[1]==args[2] else None
            if name in {'Greater','Less'}:return args[0] if args[0]==args[1] else None
            if name in {'And','Or','Not','Count'}:return 0 if all(v==0 for v in args) else None
            return args[0]
        return None
    if degree(tree)!=0:
...
def prepare(project, output, progress):
    ...
    from .selection import completed_prices, update_end_date
    prices = completed_prices(prices)
    ...
    for code, frame in prices.groupby('symbol'):
        ...
        for field in ('open','high','low','close'):
            key='raw'+field.title()
            frame['raw'+field]=pd.to_numeric(frame[key],errors='coerce') if key in frame else frame[field]/frame.factor
        frame['rawvolume']=frame.volume
    ...
    fields = sorted({c.lower() for c in numeric if c in prices} | {'factor', 'change', 'vwap', 'buyblocked', 'sellblocked', 'knownvolume','rawopen','rawhigh','rawlow','rawclose','rawvolume'})
...
def generated_path(project, value):
    """Portable candidate outputs are retained inside their owning project."""
    root=Path(project['path']).resolve();path=Path(value)
    path=(root/path).resolve() if not path.is_absolute() else path.resolve()
    if not path.is_relative_to(root) or not path.is_file():raise ValueError('生成产物须为项目目录内已保存的实际文件')
    return path


def generated_factor(project, path, factor_id):
    import pandas as pd
    from .data import symbol
    frame=pd.read_parquet(generated_path(project,path))
    ...
    if not {'datetime','instrument',factor_id}.issubset(frame):raise ValueError('生成因子缺datetime/instrument或对应因子列: '+factor_id)
    frame['datetime']=pd.to_datetime(frame.datetime,errors='raise').dt.normalize()
    frame['instrument']=frame.instrument.map(symbol)
    if frame.datetime.isna().any() or frame.duplicated(['datetime','instrument']).any():raise ValueError('生成因子日期缺失或证券日期重复')
    frame[factor_id]=pd.to_numeric(frame[factor_id],errors='raise')
    return frame.set_index(['datetime','instrument'])[[factor_id]].sort_index()


def features(project, params, prices):
    import pandas as pd
    from qlib.data import D
    if any(item.get('formula') for item in params.get('customFactors',[])):
        from .formulas import refresh_factors
    ...
    for custom in params.get('customFactors', []):
        ...
        if custom.get('source')=='parquet':
            generated[custom['id']]=generated_path(project,custom['dataPath'])
            catalog[custom['id']]='generated:'+generated[custom['id']].relative_to(Path(project['path']).resolve()).as_posix()
        else:catalog[custom['id']] = validate_expression(custom['expression'])
    ...
    for name,path in generated.items():
        if name in ids:
            values=generated_factor(project,path,name)
            custom=next(item for item in params.get('customFactors',[]) if item['id']==name)
            if custom.get('codePath'):
                code=generated_path(project,custom['codePath'])
                if values.empty or pd.Timestamp(end)>values.index.get_level_values(0).max() or code.stat().st_mtime_ns>path.stat().st_mtime_ns:
                    from .rd_agent import execute_retained_factor
                    from .storage import identifier
                    values=execute_retained_factor(project,custom,prices,end,cache/'native'/identifier())
    ...
    expected_index,expected=expected_rows(project,prices,sorted(frame.index.get_level_values(0).unique()),query_panel)
    frame=frame.reindex(expected_index)
    result, coverage = process(project, frame.replace([float('inf'), -float('inf')], float('nan')), prices, params.get('factorProcessing'),expected)
...
def analyze(project, params, output, progress, prices=None):
    ...
    for index, name in enumerate(values.columns):
        ...
        from .diagnostics import factor_tables
        for suffix,table in factor_tables(project,values[name],clean,forward,prices,ic,rank_ic).items():
            artifacts.append(save_table(output,f'{name}_{suffix}',table))
    ...
    daily_correlation=values.groupby(level=0).corr().rename_axis(index=['date','factor']).stack(dropna=False).rename('correlation')
    daily_correlation.index.names=['date','factor','otherFactor']
    artifacts.append(save_table(output,'factor_correlation_daily',daily_correlation.to_frame()))
...
def _train_single(project, params, output, progress, prices=None):
    ...
    for name in (['valid'] if params.get('_validationOnly') else ['valid', 'test']):
        ...
        predictions = pd.DataFrame({'score':estimator.predict(trading_x) if len(trading_x) else []},index=trading_x.index)
        predictions['label'] = part.label.reindex(trading_x.index)
        artifacts.append(save_table(output, name + '_predictions', predictions))
        artifacts.append(save_table(output,name+'_diagnostics',diagnostics.predictions(predictions)))
        artifacts.append(save_table(output,name+'_prediction_contributions',diagnostics.prediction_contributions(estimator,trading_x,params)))
...
def train(project, params, output, progress, prices=None):
    if params.get('model')=='native_torch':
        from .rd_agent import train_native
        return train_native(project,params,output,progress,prices)
    import pandas as pd
...
def evaluate_generated_predictions(project, params, output, progress, prices=None):
    """Evaluate retained native model output without fitting a substitute model.

    Candidate feedback is validation-only. Independent test evaluation is opt-in,
    and uses the identical raw-account backtest through a file-backed store adapter.
    """
    import pandas as pd
    import numpy as np
    from sklearn.metrics import mean_squared_error, r2_score
    from .processing import label_prices
    from .diagnostics import predictions as prediction_diagnostics
    from .data import symbol
    output=Path(output);output.mkdir(parents=True,exist_ok=True)
    bounds={name:(pd.Timestamp(params[name+'Start']),pd.Timestamp(params[name+'End'])) for name in ('train','valid','test')}
    a,b,c=bounds.values()
    if any(pd.isna(v) for pair in bounds.values() for v in pair) or not a[0]<=a[1]<b[0]<=b[1]<c[0]<=c[1]:
        raise ValueError('生成模型训练/验证/测试时间必须严格分离')
    horizon=int(params.get('labelHorizon',5));mode=params.get('labelMode','next_open')
    if not 1<=horizon<=252:raise ValueError('标签周期必须为1至252')
    source=generated_path(project,params['dataPath'])
    frame=pd.read_parquet(source)
    if isinstance(frame.index,pd.MultiIndex):frame=frame.reset_index()
    frame=frame.rename(columns={'datetime':'date','instrument':'symbol','score':'prediction'})
    required={'date','symbol','prediction','label','partition'}
    if not required.issubset(frame):raise ValueError('生成预测缺date/symbol/prediction/label/partition列')
    frame['date']=pd.to_datetime(frame.date,errors='raise').dt.normalize();frame['symbol']=frame.symbol.map(symbol)
    if frame.date.isna().any() or frame.duplicated(['date','symbol']).any():raise ValueError('生成预测日期缺失或证券日期重复')
    if not frame.partition.isin(bounds).all():raise ValueError('生成预测partition只允许train/valid/test')
    for name,pair in bounds.items():
        if not frame.loc[frame.partition.eq(name),'date'].between(*pair).all():raise ValueError('生成预测越过声明的'+name+'时间区间')
    frame['prediction']=pd.to_numeric(frame.prediction,errors='raise')
    frame['label']=pd.to_numeric(frame.label,errors='raise')
    if not np.isfinite(frame.prediction).all():raise ValueError('生成预测含非有限数值')
    prices=prepare(project,output,progress) if prices is None else prices
    artifacts=[];metrics={};coverage=[]
    partitions=['valid','test'] if params.get('includeTest',False) else ['valid']
    for name in partitions:
        part=frame[frame.partition.eq(name)].set_index(['date','symbol']).sort_index()
        if part.empty:raise ValueError('生成模型没有'+name+'预测')
        market=label_prices(prices[prices.date.le(bounds[name][1])],mode)
        labels=(market.shift(-horizon)/market-1).stack().rename('label')
        aligned=labels.reindex(part.index)
        source_labels=part.label
        checked=aligned.notna()&source_labels.notna()
        if not np.allclose(aligned[checked],source_labels[checked],rtol=1e-5,atol=1e-8):
            raise ValueError('生成模型'+name+'标签与V3的'+mode+'时点/期限口径不一致')
        table=pd.DataFrame({'score':part.prediction,'label':aligned,'sourceLabel':source_labels})
        table.index.names=['datetime','instrument']
        usable=table.dropna(subset=['label'])
        metrics[name+':mse']=float(mean_squared_error(usable.label,usable.score)) if len(usable) else None
        metrics[name+':r2']=float(r2_score(usable.label,usable.score)) if len(usable)>=2 else None
        diag=prediction_diagnostics(table)
        metrics[name+':ic']=float(diag.pearsonIC.mean()) if diag.pearsonIC.notna().any() else None
        artifacts += [save_table(output,name+'_predictions',table),save_table(output,name+'_diagnostics',diag)]
        coverage.append(dict(partition=name,predictions=len(part),evaluated=len(usable),purgedOrUnmatured=len(part)-len(usable),
            missingSourceLabels=int(source_labels.isna().sum()),labelBasis='V3 prices with partition-end truncation',**{key:params[key] for key in params if key.endswith(('Start','End'))}))
    if params.get('trainingEventsPath'):
        events=pd.read_parquet(generated_path(project,params['trainingEventsPath']))
        artifacts.append(save_table(output,'training_events',events))
    artifacts.append(save_table(output,'generated_prediction_coverage',pd.DataFrame(coverage)))
    if params.get('evaluationBacktest') is not None:
        class GeneratedStore:
            def experiment(self, project_id, experiment_id):
                if project_id!=project['id'] or experiment_id!='generated-output':raise ValueError('生成预测引用不匹配')
                return dict(kind='model.train',parameters={**params,'validation':{'mode':'single'}},artifacts=artifacts)
            def artifact_path(self, project_id, artifact):return Path(artifact['path'])
        for name in partitions:
            folder=output/(name+'_backtest');folder.mkdir(exist_ok=True)
            config={**params['evaluationBacktest'],'template':'model_score','modelExperimentId':'generated-output',
                '_predictionPartition':name,'startDate':str(bounds[name][0].date()),'endDate':str(bounds[name][1].date())}
            result=backtest(project,config,folder,lambda *_:None,prices=prices,store=GeneratedStore())
            metrics.update({name+':backtest:'+key:value for key,value in result['metrics'].items()})
            artifacts.extend({**artifact,'name':name+'_backtest_'+artifact['name']} for artifact in result['artifacts'])
    feedback={key:value for key,value in metrics.items() if key.startswith('valid:')}
    progress(.95,'原生生成模型验证评估已保存')
    return dict(metrics=metrics,artifacts=artifacts,parameters=params,summary='原生生成模型输出评估',
        details=dict(engine='native_generated_predictions',feedback=feedback,feedbackPartition='valid',
            independentTestIncluded=bool(params.get('includeTest',False)),predictionSource=str(source),
            labelHorizon=horizon,labelMode=mode,modelImportance='unavailable: native output did not provide an explanation artifact',
            trainingBoundaryVerification='declared strict split; native training code and provenance retained in candidate directory'))
...
def backtest(project, params, output, progress, prices=None, store=None):
    ...
    if template == 'model_score':
        ...
        independent_rules=bool(params.get('dailyCode') or params.get('rules')) and not params.get('factorIds')
        if independent_rules:
            ...
            if project['universe'].get('query'):
                from .market import dated_panel
                query_panel=dated_panel(project,prices)
            index,_=expected_rows(project,prices,sorted(prices.date.unique()),query_panel)
            values=pd.DataFrame(index=index)
        else:values = features(project, params, prices)
        if template == 'single_factor' and len(values.columns) != 1 and not independent_rules:
            raise ValueError('单因子模板必须选择一个因子')
        ranks = values.groupby(level=0).rank(pct=True)
        weights = params.get('weights', {})
        score = ranks.mul(pd.Series({name: float(weights.get(name, 1)) for name in ranks.columns})).sum(axis=1, min_count=len(ranks.columns)) if len(ranks.columns) else pd.Series(index=values.index,dtype=float)
    else:
    ...
    if score.empty and not (params.get('dailyCode') or params.get('rules')):
        raise ValueError('没有有效信号')
    start = max(score.index.get_level_values(0).min() if not score.empty else dates[0], pd.Timestamp(params.get('startDate') or project.get('startDate') or dates[0]))
    ...
    for row in trade_rows:
        ...
        row.update(adjustedAmount=row['amount']/factor,adjustedPrice=row['price']*factor,factor=factor)
    ...
    for date, position in positions.items():
        for code in position.get_stock_list():
            ...
            owned=daily['accountSnapshots'][date]['holdings'][code]
            holdings.append(dict(date=str(date), symbol=code, amount=owned['quantity'],quantity=owned['quantity'],sellableQuantity=owned['sellableQuantity'],
                                 pendingQuantity=owned.get('pendingQuantity',0),economicAmount=position.get_stock_amount(code),
    ...
    market_close = prices.set_index(['date','symbol']).rawClose
    prior_amounts = {r['symbol']:r['quantity'] for r in params.get('initialPositions',[])}
        ...
        for code in names:
            ...
            for trade in trades_today:
                if trade['symbol']==code:
                    pnl += (1 if trade['direction'] else -1)*trade['amount']*(close-trade['price'])-trade['cost']
    ...
    observed = reconciled.groupby('date').pnl.sum() if not reconciled.empty else pd.Series(dtype=float)
    prior_accounts = report.account.shift(1)
    prior_accounts.iloc[0] = report.account.iloc[0]/(1+net.iloc[0])
    reconciliation = pd.DataFrame({'accountPnl':report.account-prior_accounts,'securityPnl':observed.reindex(report.index,fill_value=0)})
    reconciliation['corporateAndOtherResidual']=reconciliation.accountPnl-reconciliation.securityPnl
    artifacts.append(save_table(output,'pnl_reconciliation',reconciliation))
    from .attribution import analyze as attribute
    attribution_tables=attribute(project,prices,holding_frame,report,{r['symbol']:r['quantity'] for r in params.get('initialPositions',[])},portfolio_config,pd.DataFrame(trade_rows),benchmark_name)
    ...
    return dict(metrics=metrics, artifacts=artifacts, summary='Qlib 日期规则组合回测',parameters={**params,'costs':costs,'portfolio':portfolio_config,'benchmark':benchmark_name,'topN':top_n},
                details={'engine': 'pyqlib', 'accountingBasis':'raw_shares_cash', 'accountState':daily['account'], 'annualizationDays':252, 'annualizedReturnMethod':'arithmetic', 'benchmark': benchmark_name, 'signalTiming': 'previous trading session -> next session open', 'rebalance': frequency,
                         ...
                         'warnings': list(dict.fromkeys(portfolio_warnings))+['日线开盘价近似开盘成交，非逐笔9:25集合报价；旧制IPO缺发行价时不成交。', '原始参考昨收、历史ST/上市日期不足的边界不会自动补齐。'] +
                                     (['用户 Python 代码在本地 worker 中运行；传入行情已按信号日截断。'] if params.get('code') or params.get('dailyCode') else [])})
