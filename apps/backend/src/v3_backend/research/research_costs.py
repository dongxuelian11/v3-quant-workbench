...
def reproduction_budget(store, run):
    specs = [job['spec'] for job in store.list('job', run['projectId'])
             if job.get('spec', {}).get('reproduction', {}).get('runId') == run['id']]
    result=dict(trialLimit=20, reserved=work_counts(specs),
                planned=work_counts(step['spec'] for step in run['planSnapshot']['steps']),
                message='试参按已提交次数预留，失败与重新执行不返还；训练、回测单列，滚动窗口数量以实际实验为准。')
    if run.get('budgetId'):
