...
def process(project, values, prices, config=None, expected=None):
    from sklearn.linear_model import LinearRegression
    from .history import industries
    config = {**DEFAULTS, **(config or {})}
    panels, coverage = [], []
    market = prices.set_index(['date', 'symbol'])
    expected={} if expected is None else expected.set_index('date').to_dict('index')
    for date, panel in values.groupby(level=0):
        ...
        if config['neutralizeSize']:
            ...
            basis = None
        for name in cross:
            raw_available=int(cross[name].notna().sum())
            raw_summary={key:float(value) for key,value in cross[name].agg(['min','median','max','std']).items()}
            ...
            winsor_available=int(series.notna().sum())
            ...
            neutral_available=int(series.notna().sum())
            ...
            coverage.append(dict(date=date, factor=name, available=int(series.notna().sum()), total=len(series), status=status, marketCapBasis=basis,
                inputAvailable=raw_available,winsorAvailable=winsor_available,neutralizedAvailable=neutral_available,processedAvailable=int(series.notna().sum()),
                inputMissing=len(series)-raw_available,winsorLoss=raw_available-winsor_available,neutralizationLoss=winsor_available-neutral_available,
                standardizationLoss=neutral_available-int(series.notna().sum()),
                **{'input'+key.title():value for key,value in raw_summary.items()},
                **{'processed'+key.title():float(value) for key,value in series.agg(['min','median','max','std']).items()},
                expected=expected.get(date,{}).get('expected'),denominatorStatus=expected.get(date,{}).get('denominatorStatus','factor_rows_only'),
                denominator=expected.get(date,{}).get('denominator','selected_universe_factor_rows_before_processing')))
        cross['datetime'] = date
