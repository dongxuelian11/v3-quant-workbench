...
export async function readResearchPackageDirectory(
  ...
  const descriptorPaths: unknown[] = [
    (manifest.run_spec_artifact as Record<string, unknown> | undefined)?.path,
    (manifest.execution_context_artifact as Record<string, unknown> | undefined)?.path
  ];
  for (const entry of Array.isArray(manifest.artifacts) ? (manifest.artifacts as unknown[]) : []) {
    descriptorPaths.push((entry as Record<string, unknown> | null)?.path);
  }
  for (const path of descriptorPaths) {
    if (typeof path !== "string") continue;
    if (!PACKAGE_PATH_PATTERN.test(path)) {
      throw new ProductAdapterError("INVALID_ARGUMENT", `研究包声明了非法的文件名: ${path}`);
    }
    declared.add(path);
  }
  ...
  const missing = [...declared].filter((path) => !present.has(path));
  ...
  for (const path of [...declared].sort()) {
    const payload = await readFile(join(directory, path));
    if (payload.byteLength < 1 || payload.byteLength > MAX_PACKAGE_FILE_BYTES) {
      throw new ProductAdapterError("UNBOUNDED", `研究包文件大小越界: ${path}`);
    }
