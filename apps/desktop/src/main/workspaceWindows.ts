...
export class WorkspaceWindows {
  ...
  constructor(
    ...
    private readonly keepInBackground?: () => boolean,
  ) {}

  async start(): Promise<BrowserWindow> {
    let saved: WorkspaceWindow[] = [];
    try {
      const value: unknown = JSON.parse(await readFile(this.file, "utf8"));
      if (Array.isArray(value)) saved = value.filter((item): item is WorkspaceWindow =>
    ...
    const previous = saved.find(item => item.id === "main");
    // Cold starts retain native geometry; documents are opened by the user.
    // Explicit layout presets still use restore(), including secondary windows.
    return this.open({ id: "main", main: true, panels: [],
      bounds: previous?.bounds, maximized: previous?.maximized });
  }
  ...
  private async open(state: WorkspaceWindow): Promise<BrowserWindow> {
    ...
    const visibility = () => {
      if (!window.isDestroyed()) window.webContents.send("research:workspace-changed", {
        method: "window.visibility", params: { visible: window.isVisible() && !window.isMinimized(), minimized: window.isMinimized() },
      });
    };
    ...
    window.on("close", event => {
      ...
      if (state.main) {
        if (!this.quitting && this.keepInBackground?.()) {
  ...
  current(contents: WebContents) {
    ...
    return { id: record.state.id, main: !!record.state.main, state: record.state,
      visible: record.window.isVisible() && !record.window.isMinimized(), minimized: record.window.isMinimized() };
  }
  ...
  private hiddenWindows: string[] = [];

  hide(): void {
    this.hiddenWindows = [...this.records.values()]
      .filter(record => !record.window.isDestroyed() && record.window.isVisible())
      .map(record => record.state.id);
    for (const id of this.hiddenWindows) this.records.get(id)?.window.hide();
  }

  show(): void {
    const ids = new Set(["main", ...this.hiddenWindows]);
    for (const id of ids) {
      const target = this.records.get(id)?.window;
