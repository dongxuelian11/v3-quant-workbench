...
export function FactorWorkbench({ fixtureMode }: { fixtureMode: boolean }) {
  ...
  return <section className="factor-workbench" data-testid="factor-workbench" data-fixture-mode={fixtureMode ? "DEVELOPMENT_INTEGRATION_FIXTURE" : "LIVE_READ_ONLY"}>
    <header className="factor-product-bar">
      <div><small>研究实验室 · 因子工作区</small><h1>因子研究</h1><span>定义、评估与来源保持精确绑定</span></div>
      ...
      <span className={`connection-badge ${fixtureMode ? "fixture" : "unavailable"}`}>{fixtureMode ? "开发集成数据 · DEVELOPMENT_INTEGRATION_FIXTURE" : "尚未接入 · NOT_CONNECTED"}</span>
    </header>
...
function FactorLibrary({ fixtureMode }: { fixtureMode: boolean }) {
  ...
  return <div className="factor-library" data-testid="factor-library">
    <aside className="factor-categories v3-scroll-surface" data-scroll-surface="factor-categories" aria-label="因子分类"><div className="factor-pane-title"><b>分类与视图</b><small>仅显示当前数据支持项</small></div>{categories.map((item) => {
      ...
      return <button key={item} className={category === item ? "active" : ""} disabled={!supported} onClick={() => setCategory(item)}><span>{categoryLabel(item)}</span>{supported && fixtureMode ? <small>{item === "全部" || item === "我的因子" || item === "候选" ? 5 : 4}</small> : <small>—</small>}</button>;
    })}</aside>
    <section className="factor-list-pane v3-scroll-surface" data-scroll-surface="factor-list">
      ...
      {!fixtureMode ? <TruthEmpty title="因子目录尚未接入" detail="当前 main 没有桌面只读目录路由；未注入任何演示因子。"/> : <div className="factor-list" role="listbox" aria-label="因子列表">{filtered.map((factor) => <button role="option" aria-selected={factor.assetKey === selected?.assetKey} key={factor.assetKey} onClick={() => setSelectedKey(factor.assetKey)}>
        <span className="factor-glyph">{factor.outputType === "BOOLEAN_SERIES" ? "ƒ?" : "ƒx"}</span><span><b>{factor.name}</b><code>{factor.assetKey}</code><small>TDX 用户公式 · 1d · 回看 {factor.lookback}</small></span><span><em>候选 · CANDIDATE</em><small>{factor.outputType}</small><small>未评估</small></span>
      </button>)}</div>}
    </section>
...
function FactorDetail({ factor }: { factor: FactorRecord }) {
  ...
  return <div className="factor-detail" data-factor-key={factor.assetKey}>
    <header><div><small>因子资产 · CANDIDATE</small><h2>{factor.name}</h2><code>{factor.assetKey}</code></div><span className="truth-state candidate">候选 · 非正式真值</span></header>
    ...
    <div className="factor-detail-body">
      {tab === "概览" && <><FactGrid facts={[['来源族','TDX 用户公式 · TDX_USER_FORMULA'],['生命周期','候选 · CANDIDATE'],['输出类型',factor.outputType],['频率','1d'],['回看窗口 · lookback',String(factor.lookback)],['兼容性','支持 · SUPPORTED / 精确 W0 fixture']]}/><TruthCallout tone="warning" title="未评估" detail="没有标的池 · Universe、周期、horizon/label 与 Evaluation ID；因此不展示 IC/ICIR。"/></>}
      {tab === "定义" && <><IdBlock label="因子定义版本 · FactorDefinitionVersion" value={factor.definitionVersionId}/><pre>{USER_TDX_FORMULA}</pre><FactGrid facts={[['算子 · Operators',factor.operators.join(', ')],['数据依赖','amount, volume'],['来源语言 · Source language','TDX'],['规范中间表示 · Canonical IR','由 current-main W0 translator 生成；UI 不计算']]}/></>}
      {tab === "评估" && <TruthEmpty title="未评估" detail="评估上下文 · Evaluation context 不存在：Universe / period / horizon / label / Evaluation ID 均不可用。"/>}
      {tab === "实验" && <TruthEmpty title="没有绑定实验" detail="实验必须绑定精确 FactorDefinitionVersion；当前开发数据未声明 Experiment。"/>}
      {tab === "证据 / Reviewer" && <TruthEmpty title="评审器 · Reviewer：NOT_RUN" detail="没有 ReviewerFinding；不会用 UI 状态推断评审通过。"/>}
      {tab === "版本 / 来源" && <>
        <IdBlock label="因子资产版本 · FactorAssetVersion" value={factor.assetVersionId}/>
        <IdBlock label="因子定义版本 · FactorDefinitionVersion" value={factor.definitionVersionId}/>
        <IdBlock label="因子导入收据 · FactorImportReceipt" value={factor.importReceiptId}/>
        <IdBlock label="公式文档版本 · FormulaDocumentVersion" value={FORMULA_DOCUMENT_ID}/>
        <IdBlock label="兼容性配置 · CompatibilityProfile" value={COMPATIBILITY_PROFILE_ID}/>
        <IdBlock label="数据语义配置 · DataSemanticProfile" value={DATA_PROFILE_ID}/>
        <FactGrid facts={[["来源语言","TDX"],["输出类型",factor.outputType],["规范中间表示 · Canonical IR","current-main W0 translator 生成；UI 不计算"],["评审器 · Reviewer","NOT_RUN"]]}/>
        <TruthCallout tone="warning" title="版本链只读" detail="这些精确 ID 来自 current-main W0 fixture；候选状态不等于规范准入 · canonical admission。"/>
      </>}
    </div>
  </div>;
}
...
function contractFixtureAnalysis(source: string, fixtureMode: boolean): FixtureAnalysis {
  ...
  if (source === USER_TDX_FORMULA) return { state: "PASSED", title: "W0 合同数据 · 已验证", detail: "current-main W0 测试生成的确定性投影；不是浏览器内解析结果。" };
  if (source === UNSUPPORTED_TDX_FIXTURE) return { state: "UNSUPPORTED", title: "不支持函数 · EMA", detail: "UNSUPPORTED_CANONICAL_OPERATOR（current-main W0 合同数据）" };
...
function TdxEditor({ fixtureMode }: { fixtureMode: boolean }) {
  ...
  return <div className="tdx-workspace" data-testid="tdx-editor" data-parse-state={analysis.state}>
    <section className="tdx-editor-pane"><header><div><small>TDX 源码 · L1 DRAFT</small><b>用户公式</b></div><div><button onClick={() => setSource(USER_TDX_FORMULA)}>载入 W0 开发数据</button><button onClick={() => setSource(UNSUPPORTED_TDX_FIXTURE)}>查看不支持状态</button></div></header><FormulaEditor source={source} onChange={setSource}/><footer><span>UTF-8 · 中文标识符</span><span>仅编辑；无 JS/TS 公式 VM</span></footer></section>
    <aside className="tdx-analysis-pane v3-scroll-surface" data-scroll-surface="tdx-analysis"><header><small>确定性验证预览</small><h2>{analysis.title}</h2><p>{analysis.detail}</p></header>
      {analysis.state === "PASSED" ? <><section><h3>命名输出</h3><div className="output-grid">{W0_FACTORS.map((item) => <div key={item.name}><b>{item.name}</b><span>{item.outputType}</span><small>{item.name === "GOLDEN" ? "布尔 / 信号兼容（非 1/0）" : `数值序列 · 回看 ${item.lookback}`}</small></div>)}</div></section><section><h3>静态分析</h3><FactGrid facts={[["最大回看 · lookback","60"],["数据依赖","amount, volume"],["算子 · Operator","AND, CROSS, DIVIDE, GT, MULTIPLY, SMA"],["不支持项 · Unsupported","无"]]}/></section><section><h3>TDX 数据语义</h3><p><code>VOL</code> = 手（100 股）；规范股数 · canonical shares × 0.01 → hands。<code>AMOUNT</code> = CNY。原始 <code>AMOUNT/VOL/100</code> 保持不变。</p><IdBlock label="兼容性配置 · CompatibilityProfile" value={COMPATIBILITY_PROFILE_ID}/><IdBlock label="数据语义配置 · DataSemanticProfile" value={DATA_PROFILE_ID}/></section></> : <TruthCallout tone={analysis.state === "UNSUPPORTED" ? "danger" : "warning"} title={analysis.state} detail="没有生成 Canonical IR、FactorDefinitionVersion 或执行结果。"/>}
    </aside>
  </div>;
}
...
function AiDraft({ fixtureMode }: { fixtureMode: boolean }) {
  ...
  return <div className="ai-draft-workspace" data-testid="ai-factor-draft" data-authority="L1_DRAFT">
    <section className="draft-request v3-scroll-surface" data-scroll-surface="ai-draft-request"><small>自然语言 · L1</small><h2>自然语言描述</h2><textarea value={description} onChange={(event) => setDescription(event.target.value)} aria-label="自然语言因子描述"/><button disabled={!fixtureMode} onClick={() => { setDraftVisible(true); setReceipt("AI 草案已载入；必须由用户确认"); }}><Icon name="pulse"/>生成 AI 草案</button><p>{fixtureMode ? "开发集成数据：不会调用模型或创建正式因子。" : "尚未接入 · NOT_CONNECTED：current main 没有 P Factor Agent API。"}</p>
    ...
    <section className="draft-review v3-scroll-surface" data-scroll-surface="ai-draft-review">{draftVisible ? <><header><div><span className="truth-state draft">AI 提案 · DRAFT</span><h2>黄金交叉量价草案</h2><p>非规范 FactorDefinitionVersion · 需要用户确认</p></div><span className="confirm-required">需要用户确认</span></header><pre>{USER_TDX_FORMULA}</pre><div className="draft-flow"><span>自然语言描述</span><i>→</i><span className="active">AI 草案</span><i>→</i><span>确定性验证预览</span><i>→</i><span>用户确认</span></div><TruthCallout tone="warning" title="验证边界" detail="当前内容只映射到已知 W0 开发数据；没有后端创建路由，不会生成规范因子 · canonical factor。"/><footer><span role="status">{receipt}</span><button onClick={() => setReceipt("用户已确认审阅意图；仍为 L1 DRAFT / NOT_CONNECTED")}>确认审阅意图（L1）</button><button disabled>执行（L2 不可用）</button><button disabled>发布（L3 不可用）</button></footer></> : <TruthEmpty title="AI 因子智能体尚未接入" detail="不会导入未合并 P 分支，也不会在生产中回退到开发数据。"/>}</section>
  </div>;
}
...
function IdBlock({ label, value }: { label: string; value: string }) {
  ...
  return <div className="id-block"><small>{label}</small><code title={value}>{value}</code><button aria-label={`复制${label}`} onClick={() => { void navigator.clipboard?.writeText(value); setCopied(true); }}>{copied ? "已复制" : "复制"}</button></div>;
}
...
function categoryLabel(value: string): string {
  const labels: Record<string, string> = {
    price: "价格 · price", volume: "成交量 · volume", momentum: "动量 · momentum", reversal: "反转 · reversal",
    volatility: "波动率 · volatility", liquidity: "流动性 · liquidity", value: "价值 · value", quality: "质量 · quality",
    growth: "成长 · growth", profitability: "盈利 · profitability", sentiment: "情绪 · sentiment", technical: "技术 · technical", custom: "自定义 · custom"
  };
