...
export function AiPanel({ settingsOpen, closeSettings, hidden = false }: { settingsOpen: boolean; closeSettings: () => void; hidden?: boolean }) {
  ...
  useEffect(() => { setMessages([]); setAnswer(null); setStageJobs([]); setInput(""); }, [s.project?.id]);
  ...
  async function send(text = input) {
    if (!s.project || !text.trim() || busy) return;
    const projectId = s.project.id; const history = messages;
    setBusy(true); setInput(""); setMessages([...history, { role: "user", content: text }]);
    await s.act(async () => {
      const response = await request<Answer>("ai.chat", { projectId, mode, message: text, history });
      setAnswer(response); setMessages([...history, { role: "user", content: text }, { role: "assistant", content: response.message }]);
    });
    setBusy(false);
  }
  async function runStage() {
    if (!answer || !s.project) return;
    ...
    await s.act(async () => {
      for (const proposal of proposals) { const event = await s.submit(proposal.spec.kind, proposal.spec.parameters, proposal.spec.name ?? proposal.title); submitted.push(event.id); setAnswer(current => current ? { ...current, proposals: current.proposals.filter(p => p !== proposal) } : current); }
    });
    setStageJobs(submitted); setBusy(false);
  }
  return <aside className="r-ai" style={hidden ? { display: "none" } : undefined}><div className="r-toolbar"><h2>AI 研究助手</h2><span className="r-tag">辅助研究</span></div><nav className="r-subtabs" aria-label="AI 模式">{([ ["ask", "问答"], ["assist", "辅助配置"], ["research", "自主研究"] ] as const).map(([id, title]) => <button key={id} className={mode === id ? "active" : ""} onClick={() => setMode(id)}>{title}</button>)}</nav>
    {showSettings ? <div className="r-ai-settings"><h3>模型设置</h3><div className="r-toolbar"><button className={endpoint === "online" ? "active" : ""} onClick={() => setEndpoint("online")}>在线兼容服务</button><button className={endpoint === "local" ? "active" : ""} onClick={() => setEndpoint("local")}>本地服务</button></div>{settings && <><Field label={endpoint === "local" ? "本地服务地址" : "兼容 API 地址"}><input placeholder={endpoint === "local" ? "http://localhost:8000/v1" : "https://…/v1"} value={settings.ai.baseUrl} onChange={e => setSettings({ ...settings, ai: { ...settings.ai, baseUrl: e.target.value } })} /></Field><Field label="模型名称"><input value={settings.ai.model} onChange={e => setSettings({ ...settings, ai: { ...settings.ai, model: e.target.value } })} /></Field><Field label="API Key"><input autoComplete="off" type="password" value={settings.ai.apiKey} onChange={e => setSettings({ ...settings, ai: { ...settings.ai, apiKey: e.target.value } })} /></Field><Field label="Temperature"><input type="number" min={0} max={2} step={0.1} value={settings.ai.temperature} onChange={e => setSettings({ ...settings, ai: { ...settings.ai, temperature: Number(e.target.value) } })} /></Field><Field label="默认数据源"><select value={settings.defaultDataSource} onChange={e => setSettings({ ...settings, defaultDataSource: e.target.value as Settings["defaultDataSource"] })}><option value="baostock">BaoStock</option><option value="akshare">AKShare</option></select></Field><button className="r-primary" onClick={() => void s.act(async () => { await request("settings.save", { settings }); setShowSettings(false); closeSettings(); s.setNotice("模型设置已保存；连接情况以实际请求结果为准"); })}>保存设置</button></>}<button onClick={() => { setShowSettings(false); closeSettings(); }}>返回对话</button><p className="r-note">本地模型需先启动兼容服务，模型权重不随桌面程序分发。</p></div> : <><div className="r-chat-log" aria-live="polite">{!messages.length && <><div className="r-chat-intro"><span className="r-ai-symbol">◎</span><p>围绕你的研究目标，挑选因子、调整参数，或解释已有实验。</p></div><h3>你可以尝试</h3><button className="r-suggestion" disabled={!s.project} onClick={() => setInput("请根据项目目标规划一次因子比较，先讨论方案。")}>规划一次因子比较 <span>→</span></button><button className="r-suggestion" disabled={!s.project} onClick={() => setInput("请解释当前项目最近实验的结果、局限和下一步。")}>解释实验结果 <span>→</span></button></>}{messages.map((m, i) => <div key={i} className={`r-message ${m.role}`}><small>{m.role === "user" ? "你" : "研究助手"}</small><p>{m.content}</p></div>)}{busy && <p role="status">正在处理…</p>}{answer?.phase && <p className="r-note">研究阶段：{answer.phase}</p>}{answer?.proposals.map((proposal, i) => <section className="r-proposal" key={i}><h3>{proposal.title}</h3><p>{proposal.description}</p><details><summary>查看参数</summary><pre>{JSON.stringify(proposal.spec.parameters, null, 2)}</pre></details>{mode === "assist" && <button onClick={() => void s.act(async () => { const key = proposal.spec.kind === "model.train" ? "model" : proposal.spec.kind === "backtest.run" ? "backtest" : null; if (!key) throw new Error("此方案可在研究模式中运行，不能应用到参数表单。"); await s.save({ settings: { ...s.project!.settings, [key]: proposal.spec.parameters } }); const ids = proposal.spec.parameters.factorIds; if (Array.isArray(ids)) s.setSelectedFactors(ids.filter((v): v is string => typeof v === "string")); s.setPage(key === "model" ? "model" : "backtest"); s.setNotice("已应用方案参数，请查看后运行"); })}>应用参数</button>}</section>)}{mode === "research" && !!answer?.proposals.length && <button className="r-primary" disabled={busy || stageJobs.length > 0 && !terminal} onClick={() => void runStage()}>运行这一阶段（{answer.proposals.length} 项）</button>}{terminal && <button className="r-primary" disabled={busy} onClick={() => void send(`本阶段任务已结束。请读取这些任务及其结果，讨论下一阶段，先不要执行：${stageJobs.join(", ")}`)}>携本阶段结果继续讨论</button>}</div><form className="r-chat-compose" onSubmit={e => { e.preventDefault(); void send(); }}><textarea aria-label="研究问题" placeholder={s.project ? "描述你的研究问题…" : "先创建或打开一个项目"} value={input} onChange={e => setInput(e.target.value)} disabled={!s.project} /><button type="submit" className="r-send" aria-label="发送研究问题" disabled={busy || !input.trim() || !s.project}>→</button></form><div className="r-toolbar r-ai-footer"><button onClick={() => setShowSettings(true)}>模型设置</button><span>{mode === "research" ? "每阶段结束后讨论" : "结果以实验为准"}</span></div></>}
  </aside>;
}
