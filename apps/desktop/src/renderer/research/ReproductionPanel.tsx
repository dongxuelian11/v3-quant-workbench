...
export function ReproductionPanel({ panel }: { panel: WorkspacePanel }) {
  ...
  <div className="r-toolbar">
    ...
    {canRetryFailed(run) && <button disabled={busy} title="保留已完成步骤；新 attempt 计入预算" onClick={() => void act(async () => { await request("reproductions.run", { projectId, planId: run.planId, runId: run.runId, retryFailed: true });setRevision(v => v + 1); })}>重试失败步骤</button>}
  ...
  {run.budget&&<div className="r-note"><p>本方案试参预留 {run.budget.reserved.trials} / {run.budget.trialLimit} 次 · 计划试参 {run.budget.planned.trials} 次</p><p>已预留任务：训练 {run.budget.reserved.trainingTasks} · 回测 {run.budget.reserved.backtestTasks} · 滚动 {run.budget.reserved.rollingTasks}</p><p>{run.budget.message}</p></div>}
  <ul>{run.steps.map(step => <li key={step.stepId}>
    <span>{step.stepId} · {statuses[step.status] ?? step.status}{step.attempt !== undefined && <> · 第 {step.attempt} 次尝试</>}</span>
    ...
    {!!step.attemptHistory?.length && <details><summary>历史尝试 · {step.attemptHistory.length}</summary><ul>{step.attemptHistory.map((attempt, index) => <li key={String(attempt.attempt) + "-" + (attempt.jobId ?? index)}>第 {attempt.attempt} 次 · {statuses[attempt.status] ?? attempt.status}{attempt.experimentId && <button onClick={() => w.open({ kind: "experiment", title: "查看保留实验", projectId, experimentId: attempt.experimentId })}>查看保留实验</button>}</li>)}</ul></details>}
  </li>)}</ul>
</section>)}</>}</div>;
}
