...
export function useResearchState() {
  ...
  async function refresh(id = current.current?.id) {
    const [exps, tasks] = await Promise.all([request<Experiment[]>("experiments.list", { all: true }), Promise.all([request<JobEvent[]>("jobs.list",{statuses:["queued","running"],limit:1000}),request<JobEvent[]>("jobs.list",{statuses:["completed","failed","cancelled","interrupted"],limit:100})]).then(([active,recent])=>[...active,...recent])]);
