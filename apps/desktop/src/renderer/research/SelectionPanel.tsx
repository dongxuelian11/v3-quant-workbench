...
export function SelectionPanel() {
  ...
  return <div className="r-page"><Heading title="每日选股" description="从已确认方案生成候选、目标组合和调仓清单。清单不会改变实际持仓。" />
    <div className="r-toolbar"><span className="r-tag">{configured.enabled === true ? "方案已启用" : "尚未启用方案"}</span>{changed && <span>编辑中的方案尚未启用</span>}<span>数据日期：{valueText(result?.details.dataDate)}</span><span>模型更新时间：{valueText(result?.details.modelUpdatedAt)}</span></div>
    <div className="r-form-grid"><Field label="策略方案"><select value={strategyChoice} disabled={busy} onChange={e => void choose("strategy", e.target.value)}><option value="project">项目已保存策略</option>{s.experiments.filter(e => e.kind === "backtest.run").map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></Field><Field label="模型方案"><select value={modelChoice} disabled={busy} onChange={e => void choose("model", e.target.value)}><option value="project">项目已保存模型设置</option>{s.experiments.filter(e => e.kind === "model.train").map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></Field><Field label="数据源"><select value={source} onChange={e => { setSource(e.target.value); setChanged(true); }}><option value="baostock">BaoStock</option><option value="akshare">AKShare</option></select></Field><label className="r-check"><input type="checkbox" checked={update} onChange={e => { setUpdate(e.target.checked); setChanged(true); }} />生成前增量更新</label><label className="r-check"><input type="checkbox" checked={financials} onChange={e => { setFinancials(e.target.checked); setChanged(true); }} />更新财务数据</label></div>
    ...
    <div className="r-toolbar"><button className="r-primary" disabled={busy} onClick={() => void s.act(async () => { await s.save({ settings: { ...p.settings, selection: snapshot() } }); setChanged(false); s.setNotice("已明确启用所选策略与模型方案"); })}>启用这组方案</button><button disabled={configured.enabled !== true || busy || changed} onClick={() => { setBusy(true); void s.act(async () => { if (!savePositions.current) throw new Error("持仓仍在加载。"); await savePositions.current(); await s.submit("selection.run", object(p.settings.selection)); }).finally(() => setBusy(false)); }}>更新并生成清单</button><button onClick={() => s.setPage("strategy")}>编辑策略</button><button onClick={() => s.setPage("model")}>编辑模型</button>{configured.enabled === true && <button onClick={() => void s.act(() => s.save({ settings: { ...p.settings, selection: { ...configured, enabled: false } } }))}>停用方案</button>}</div>
    <PositionsEditor saveRef={savePositions} />
...
function PositionsEditor({ saveRef }: { saveRef: React.RefObject<(() => Promise<void>) | null> }) {
  ...
  useEffect(() => { let active = true; void s.act(async () => { const data = await request<Positions>("positions.get", { projectId: id }); if (active) setPositions(data); }); return () => { active = false; }; }, [id]);
