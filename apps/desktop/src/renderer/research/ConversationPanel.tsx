...
export function ConversationPanel(props:{settingsOpen:boolean;closeSettings:()=>void}) {
 ...
 return <>{[...sessions.current].map(([sessionKey, session])=><div key={sessionKey} hidden={sessionKey!==key} style={{height:"100%",minHeight:0}}><ProjectConversation {...props} active={sessionKey===key} projectId={sessionKey==="global"?undefined:sessionKey} session={session}/></div>)}</>;
}
function ProjectConversation({settingsOpen,closeSettings,projectId,session,active}:{active:boolean;settingsOpen:boolean;closeSettings:()=>void;projectId?:string;session:Session}) {
 ...
 const [settings,setSettings]=useState<Settings|null>(null);const [showSettings,setShowSettings]=useState(false);const [serviceStatus,setServiceStatus]=useState("正在读取服务设置");
 ...
 const execution=object(state.execution);const executionActive=(value:JsonObject)=>typeof value.id==='string'&&!['completed','failed','cancelled','interrupted','paused'].includes(String(value.status));
 ...
 useEffect(()=>{void s.act(async()=>{const config=await request<Settings>("settings.get");setSettings(config);setServiceStatus(configurationStatus(config));await refreshList();});},[]);
 ...
 useEffect(()=>{if(settingsOpen&&active)setShowSettings(true);},[settingsOpen,active]);useEffect(()=>{if(showSettings)void s.act(async()=>setSettings(await request("settings.get")));},[showSettings]);
