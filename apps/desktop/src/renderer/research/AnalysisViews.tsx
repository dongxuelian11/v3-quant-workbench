...
interface AnalysisResponse extends ResearchTable {total:number;returned:number;aggregation:{method:"complete"|"preview"|"minmax";sampled:boolean;sourceRows:number};calendarSource?:string|null;message:string;}
interface AnalysisOptions {factor?:string;rolling?:{window:number;statistic:string};}
/** One bounded backend response. Complete-window statistics are calculated before previewing. */
function useAnalysisTable(detail:ExperimentDetails,name:string,overview=false,date="",symbol="",options:AnalysisOptions={}) {
 const [table,setTable]=useState<AnalysisResponse|null>(null),[error,setError]=useState("");
 const [startDate,setStartDate]=useState(""),[endDate,setEndDate]=useState("");const [hasDates,setHasDates]=useState(false);
 const viewport=useRef<HTMLDivElement>(null),[pointBudget,setPointBudget]=useState(1200);
 useEffect(()=>{if(!overview||!viewport.current)return;let timer:ReturnType<typeof setTimeout>|undefined;const update=()=>{const width=viewport.current?.getBoundingClientRect().width??0;if(width<100)return;if(timer)clearTimeout(timer);timer=setTimeout(()=>setPointBudget(Math.max(200,Math.min(5000,Math.round(width*2/100)*100))),150);};const observer=new ResizeObserver(update);observer.observe(viewport.current);update();return()=>{observer.disconnect();if(timer)clearTimeout(timer);};},[overview]);
 const artifact=tableName(detail,name),factor=options.factor,rollingWindow=options.rolling?.window,statistic=options.rolling?.statistic;
 useEffect(()=>{let alive=true;const reads=readScope();setTable(null);setError("");if(!artifact)return;
 if(startDate&&endDate&&startDate>endDate){setError("开始日期不能晚于结束日期。");return;}
 void(async()=>{let filterDate=date,from=startDate,to=endDate;
 if(date||from||to){const probe=await reads.request<TablePage>("experiments.table",{projectId:detail.experiment.projectId,experimentId:detail.experiment.id,table:artifact,offset:0,limit:1});if(!alive)return;if(!probe.columns.some(column=>["date","datetime","trade_date"].includes(column))){filterDate="";from="";to="";}}
 const result=await reads.request<AnalysisResponse>("experiments.analysis",{projectId:detail.experiment.projectId,experimentId:detail.experiment.id,table:artifact,maxPoints:overview?pointBudget:500,...(overview?{sampling:"minmax"}:{}),...(symbol?{symbol}:{}),...(factor?{factor}:{}),...(filterDate?{startDate:filterDate,endDate:filterDate}:{...(from?{startDate:from}:{}),...(to?{endDate:to}:{})}),...(rollingWindow?{rolling:{window:rollingWindow,statistic}}:{})});if(alive){setTable(result);setHasDates(result.columns.some(column=>["date","datetime","trade_date"].includes(column)));}
 })().catch(e=>{if(alive)setError(errorText(e));});return()=>{alive=false;reads.cancel();};
 },[detail.experiment.id,detail.experiment.projectId,artifact,overview,date,symbol,factor,rollingWindow,statistic,startDate,endDate,pointBudget]);
 const canFilter=overview&&!date&&hasDates;
 const notice=<div ref={viewport} className="r-analysis-read-status">{table?.message&&<p className="r-note" role="status">{table.message}；{table.aggregation.method==="minmax"?"图表按视口抽样，仅用于绘图；统计以完整区间结果为准。":"图表为该范围预览，不代表完整曲线。"}</p>}{canFilter&&<details className="r-analysis-range"><summary>图表日期范围</summary><div className="r-toolbar"><label>开始<input aria-label="分析图开始日期" type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></label><label>结束<input aria-label="分析图结束日期" type="date" value={endDate} onChange={e=>setEndDate(e.target.value)}/></label><button onClick={()=>{setStartDate("");setEndDate("");}}>清除日期</button></div></details>}</div>;
 return {table,error,total:table?.total??0,missing:!artifact,notice};
}
...
export function FactorAnalysis({detail}:{detail:ExperimentDetails}) {
 ...
 const loaded=useAnalysisTable(detail,artifact,!['distribution','industry','correlationDaily'].includes(view)||!!date,['distribution','industry','correlationDaily','samples'].includes(view)?date:'','',{...(view==='coverage'&&factor?{factor}:{}),...(['trend','rank'].includes(view)?{rolling:{window,statistic}}:{})});
 useEffect(()=>{if(panel?.factorId&&panel.factorId!==factor)setFactor(panel.factorId);},[panel?.factorId]);
 const rows=loaded.table?.rows??[];const axis=loaded.table?.columns.find(k=>/date|datetime|quarter|period|factor_quantile/.test(k))??loaded.table?.columns[0]??'';
 ...
 const holdingPeriods=[...new Set(columns.map(key=>key.match(/^([^_]+)_Q\d+(?:\.0+)?$/)?.[1]).filter((value):value is string=>!!value))];
 ...
 {loaded.notice}{!factor&&!['coverage','correlation','correlationDaily'].includes(view)?<DataTable table={summary} onRow={row=>{setFactor(String(row.factor));choose({},{factorId:String(row.factor)});}}/>:loaded.missing?<Missing name={views.find(v=>v[0]===view)?.[1]??view}/>:loaded.error?<p role="alert">{loaded.error}</p>:!loaded.table?<p role="status">正在读取分析数据…</p>:<>
 <p className="r-note">{name} · {trend?`${window} 个${loaded.table?.calendarSource==="experiment_processing_sessions"?"交易日":"工作日（缺少交易日历）"}${statistic==="icir"?"均值 / 样本标准差":"滚动均值"}；不足完整有效窗口或标准差为零时留空。IC 与 ICIR 为无量纲统计量。`:view==='correlation'?String(detail.details.correlation??'相关性口径未记录'):view==='decay'?'期限衰减按不同持有期限的平均 IC 展示，与时间稳定性及换手率分别计算。':view==='groups'?'按原实验非重叠持有期组合的累计分组净值。':view==='distribution'?`已读取 ${rows.length} / ${loaded.total} 行${!date?'；选择日期查看当日截面。':''}`:''}</p>
...
function DailyCorrelation({rows,date,total}:{rows:JsonObject[];date:string;total:number}) {
 if(date&&rows.length<total)return <p className="r-note">该截面仅返回 {rows.length} / {total} 行，暂不绘制不完整相关矩阵；请查看完整明细。</p>;
...
export function FactorComparisonView({detail}:{detail:ExperimentDetails}) {
 ...
 const {table,error,missing,notice}=useAnalysisTable(detail,`${factor}_cumulative_quantiles`,true);
 const periods=[...new Set((table?.columns??[]).flatMap(key=>key.match(/^([^_]+)_Q\d+(?:\.0+)?$/)?.[1]??[]))];
 const selected=periods.includes(period)?period:periods[0];
 const columns=table?.columns.filter(key=>key.startsWith(`${selected}_Q`))??[];
 return <div className="r-factor-comparison-view">{notice}<div className="r-toolbar"><label>因子 <select value={factor} onChange={e=>setFactor(e.target.value)}>{factors.map(id=><option key={id}>{id}</option>)}</select></label><label>持有期 <select value={selected??''} onChange={e=>setPeriod(e.target.value)}>{periods.map(value=><option key={value}>{value}</option>)}</select></label></div>{!factor?<p>本实验未记录可分析因子。</p>:<>{error?<p role="alert">{error}</p>:missing?<p>本实验未保存分组净值。</p>:table&&columns.length?<Plot title="分组净值" option={{tooltip:{trigger:'axis'},legend:{type:'scroll'},grid:{left:45,right:12,top:30,bottom:30},xAxis:{type:'category',data:table.rows.map(dateOf)},yAxis:{type:'value',scale:true},series:columns.map(key=>({name:key,type:'line',showSymbol:false,connectNulls:false,data:table.rows.map(row=>numeric(row[key])?row[key]:null)}))}}/>:<p>{table?"本实验没有可绘制的分组净值，原始表仍可在实验页查看。":"正在读取分组数值…"}</p>}<FactorICOverview detail={detail} factor={factor} onRow={()=>{}}/><FactorCoverageOverview detail={detail} factor={factor}/></>}</div>;
}

function FactorICOverview({detail,factor,onRow}:{detail:ExperimentDetails;factor:string;onRow:(row:JsonObject)=>void}) {
 const {table,error,missing,notice}=useAnalysisTable(detail,`${factor}_RankIC`,true,"","",{rolling:{window:20,statistic:"mean"}});
 if(error)return <>{notice}<p role="alert">{error}</p></>;
 ...
 const series=keys.map(key=>({name:key,type:'line' as const,showSymbol:false,connectNulls:false,data:rows.map(row=>numeric(row[key])?row[key] as number:null)}));
 if(!series.some(item=>item.data.some(numeric)))return <>{notice}<div className="r-factor-unavailable"><h3>{keys.length?"滚动 Rank IC 暂不可用":"该实验无可用 Rank IC"}</h3><p className="r-note">{keys.length?'当前返回范围未形成完整的 20 个交易观测窗口。':'当前返回范围没有有效滚动数值，不能绘制走势。'}可在完整原始数值中核对。</p></div></>;
 return <>{notice}<Plot title={`RankIC · 20 ${table.calendarSource==="experiment_processing_sessions"?"交易日":"工作日（缺少日历）"}均值`} option={{tooltip:{trigger:'axis'},legend:{type:'scroll'},grid:{left:65,right:20,bottom:50},xAxis:{type:'category',data:rows.map(dateOf)},yAxis:{type:'value',min:-1,max:1},series}} onPoint={i=>rows[i]&&onRow(rows[i])}/></>;
}

function FactorCoverageOverview({detail,factor}:{detail:ExperimentDetails;factor:string}) {
 const {table,error,missing,notice}=useAnalysisTable(detail,"processing_coverage",true,"","",{factor});
 if(error)return <>{notice}<p role="alert">{error}</p></>;
 ...
 return <>{notice}<Plot title="处理覆盖率 · 最终有效数 / 输入总数" option={{tooltip:{trigger:'axis'},grid:{left:50,right:15,bottom:40},xAxis:{type:'category',data:rows.map(dateOf)},yAxis:{type:'value',min:0,max:1},series:[{type:'line',showSymbol:false,connectNulls:false,data:rows.map(row=>numeric(row.total)&&row.total>0&&numeric(row.processedAvailable)?row.processedAvailable/row.total:null)}]}}/></>;
}
