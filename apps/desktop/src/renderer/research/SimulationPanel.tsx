...
function SimulationExports({account,view,disabled}:{account:SimulationAccount;view:string;disabled:boolean}) {
  ...
  return <details><summary>账户导出与报告</summary><div className="r-toolbar"><button disabled={disabled||busy||!snapshot} onClick={()=>exportFile("csv")}>导出{label}完整 CSV</button><button disabled={disabled||busy||!snapshot} onClick={()=>exportFile("xlsx")}>导出账户全部记录 Excel</button><button disabled={!snapshot} onClick={()=>snapshot&&w.open({kind:"experiment",title:`${account.name} · 推进结果`,projectId:account.projectId,strategyId:account.strategyId,experimentId:snapshot.id})}>查看推进结果 / PDF 报告</button></div><p className="r-note">导出最近一次推进保存的完整记录，不受表格分页和搜索影响。{!snapshot?"尚无推进结果可导出。":""}</p>{error&&<p role="alert">{error}</p>}</details>;
}
