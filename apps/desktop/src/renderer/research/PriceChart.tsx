...
export function PriceChart({ symbol: initial = "", focusDate, trades = [] }: { symbol?: string; focusDate?: string; trades?: TradeMarker[] }) {
  ...
  useEffect(() => {
    ...
    instance.setDataLoader({ getBars: async ({ type, timestamp, callback }) => {
      ...
      try {
        ...
        if (type === "init") { setCount(data.length); setStatus(data.length ? `${symbol} · 日线，可拖动加载历史` : "没有这只证券在该区间的日线数据"); if (anchor != null) instance.scrollToTimestamp(anchor); }
      } catch (e) { if (active) { callback([], false); setStatus(errorText(e)); } }
    } });
  ...
  }, [symbol, projectId, focusDate]);
  useEffect(() => {
    const instance = chart.current; if (!instance || !count) return;
    instance.removeOverlay({ groupId: "research-trade" });
    trades.forEach((trade, index) => instance.createOverlay({ name: "research_note", id: `trade-${index}`, groupId: "research-trade", lock: true, points: [{ timestamp: Date.parse(`${trade.date.slice(0, 10)}T00:00:00+08:00`), value: trade.price }], extendData: `${trade.side} · ${trade.price}`, styles: { text: { color: trade.side === "买入" ? "#b35c4f" : "#287775" } } }));
  }, [JSON.stringify(trades), count, symbol]);
