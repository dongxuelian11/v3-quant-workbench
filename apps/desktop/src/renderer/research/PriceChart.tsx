...
export function PriceChart({ symbol: initial = "", focusDate, trades = [], experimentId, tradeTable, embedded = false, asOfDate, replayMode = false, priceBasis: initialPriceBasis = "adjusted", initialStartDate, initialEndDate, focusedTrade, onTradeClick, footer, instrument, refreshVersion, onHistoryNeeded, onBarSelect, adjustedAvailable = true, annotationProjectId }: { annotationProjectId?:string; onBarSelect?: (date:string)=>void; adjustedAvailable?: boolean; onHistoryNeeded?: (beforeDate: string) => Promise<void>; instrument?: QuoteInstrument; refreshVersion?: number; symbol?: string; focusDate?: string; trades?: TradeMarker[]; experimentId?: string; tradeTable?: string; embedded?: boolean; asOfDate?: string; replayMode?: boolean; priceBasis?: "raw" | "adjusted"; initialStartDate?: string; initialEndDate?: string; focusedTrade?: TradeMarker; footer?:React.ReactNode; onTradeClick?: (trade: TradeMarker) => void }) {
  ...
  const s = useResearch(); const projectId = instrument ? undefined : s.project?.id; const drawingProjectId = annotationProjectId ?? projectId;
  ...
  return <div className={`r-chart-panel${embedded ? " r-chart-embedded" : ""}`}>
    ...
    <div className={`r-chart-canvas${toolkit.focused?" r-chart-focus":""}`} inert={rebaseBlocked.current?true:undefined} onKeyDown={toolkit.key} onMouseLeave={()=>{setHover(null);if(replayMode)requestAnimationFrame(()=>restoreFocus.current());}}><div ref={node} className="r-kline" tabIndex={0} onMouseDownCapture={toolkit.begin} onPointerDownCapture={event=>{dragged.current=false;dragOrigin.current={x:event.clientX,y:event.clientY};}} onPointerMoveCapture={event=>{if(!toolkit.tool)setHover(barAt(event));else setHover(null);const origin=dragOrigin.current;if(origin&&event.buttons&&Math.hypot(event.clientX-origin.x,event.clientY-origin.y)>6){dragged.current=true;permitHistory();dragOrigin.current=null;}}} onPointerUpCapture={event=>{if(!dragged.current&&!toolkit.tool&&event.button===0){const picked=barAt(event);if(picked)onBarSelect?.(dateAt(picked.bar.timestamp));}dragOrigin.current=null;}} onWheelCapture={permitHistory} style={{cursor:toolkit.tool?"crosshair":undefined}} role="img" aria-label={`${symbol} 日线与模拟成交图`} />{hover&&<div className="r-price-tooltip" style={{left:hover.x,top:hover.y}}><strong>{dateAt(hover.bar.timestamp)} · {replayMode?"日线":({day:"日线",week:"周线",month:"月线",trading_days:`${tradingDays}交易日`})[period]} · {priceBasis==="raw"?"不复权":"复权"}</strong><dl>{[["开盘",hover.bar.open],["最高",hover.bar.high],["最低",hover.bar.low],["收盘",hover.bar.close],["较前一根",hover.previous?hover.bar.close-hover.previous.close:undefined]].map(([label,value])=><div key={String(label)}><dt>{String(label)}</dt><dd>{hoverNumber(value)}</dd></div>)}<div><dt>较前一根（%）</dt><dd>{hover.previous&&hover.previous.close!==0?`${hoverNumber((hover.bar.close/hover.previous.close-1)*100)}%`:"—"}</dd></div><div><dt>成交量（股）</dt><dd>{hoverNumber(hover.bar.volume)}</dd></div><div><dt>成交额（元）</dt><dd>{hoverNumber(hover.bar.turnover)}</dd></div></dl></div>}{barsLoaded&&!count&&!loadFailed&&<div className="r-chart-empty" role="status">暂无此区间行情{instrument&&<small>可使用上方“更新当前行情”，或补取其他日期。</small>}</div>}{chartTools}</div>
    {footer}
  </div>;
}
