...
export function QuotePanel({ panel }: { panel: WorkspacePanel }) {
  ...
  const [kind, setKind] = useState<QuoteInstrument["kind"]>(instrument.kind);
  const [categoryViews,setCategoryViews]=useState<Partial<Record<QuoteInstrument["kind"],{query:string;offset:number;watchlistId:string}>>>({});
  ...
  const [historyStart, setHistoryStart] = useState("2015-01-01"), [listVisible, setListVisible] = useState(true);
