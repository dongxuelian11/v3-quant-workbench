...
export function ResultsPanel() {
  ...
  const exportResult = (format: "csv" | "xlsx" | "pdf") => void s.act(async () => {
    ...
    if (format !== "pdf") {
      ...
      const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><style>body{font:13px "Microsoft YaHei",sans-serif;color:#263536;padding:32px}table{border-collapse:collapse;font-size:10px;width:100%}td,th{border-bottom:1px solid #ddd;padding:5px;text-align:left}pre{white-space:pre-wrap}h1{color:#267774}</style><h1>${escapeHtml(detail.experiment.name)}</h1><p>${escapeHtml((s.project?.name ?? "全局研究"))} · ${escapeHtml(detail.experiment.createdAt)}</p><p>${escapeHtml(detail.experiment.summary)}</p><h2>指标</h2><pre>${escapeHtml(JSON.stringify(detail.experiment.metrics, null, 2))}</pre><h2>参数</h2><pre>${escapeHtml(JSON.stringify(detail.experiment.parameters, null, 2))}</pre>${tableHtml}<p>研究报告 · 表格为返回的预览行，完整数据请导出 CSV / Excel。</p></html>`;
