...
export function ResultsPanel() {
  ...
  return <div className={`r-page r-results-page${tab==="overall"&&detail?.experiment.kind==="factor.analyze"?" r-factor-result-page":""}`}><div className="r-results-layout"><div className="r-result-selector"><label className="r-field"><select aria-label="选择实验结果" value={id} onChange={e=>{s.setSelectedExperiment(e.target.value);setTab("overall");}}>{s.experiments.map(e=><option key={e.id} value={e.id}>{e.starred?"★ ":""}{jobTitle(e.kind,e.name)} · {new Date(e.createdAt).toLocaleDateString("zh-CN")}</option>)}</select></label><details className="r-result-compare-picker"><summary>实验比较{selected.length ? `（${selected.length}）` : ""}</summary><div className="r-compare-choices">{s.experiments.map(e=><label key={e.id}><input type="checkbox" checked={selected.includes(e.id)} onChange={()=>setSelected(ids=>ids.includes(e.id)?ids.filter(x=>x!==e.id):[...ids,e.id])}/><span>{jobTitle(e.kind,e.name)}</span></label>)}</div><button disabled={selected.length<2} onClick={()=>void s.act(async()=>{setComparison(await request<ExperimentDetails[]>("experiments.compare",{projectId:s.project?.id,experimentIds:selected}));setTab("compare");})}>比较已选（{selected.length}）</button></details>{detail&&<details className="r-result-more"><summary>更多操作</summary><div className="r-toolbar">{tab==="replay"&&<><button onClick={()=>exportResult("csv")}>导出 CSV</button><button onClick={()=>exportResult("xlsx")}>导出 Excel</button><button onClick={()=>exportResult("pdf")}>导出 PDF 报告</button></>}<input aria-label="实验名称" value={name} onChange={e => setName(e.target.value)} /><button disabled={!name.trim()} onClick={() => void s.act(async () => { await request("experiments.update", { projectId: s.project?.id, experimentId: id, name: name.trim() }); await s.refresh(); })}>重命名</button><button onClick={() => void s.act(async () => { await request("experiments.update", { projectId: s.project?.id, experimentId: id, starred: !detail.experiment.starred }); await s.refresh(); })}>{detail.experiment.starred ? "取消收藏" : "收藏"}</button><button className="r-danger" onClick={() => { if (window.confirm(`删除实验“${detail.experiment.name}”？`)) void s.act(async () => { await request("experiments.delete", { projectId: s.project?.id, experimentId: id }); s.setSelectedExperiment(""); setComparison([]); setSelected(ids => ids.filter(x => x !== id)); await s.refresh(); }); }}>删除</button></div></details>}</div><section className="r-result-content">{loading ? <Empty title="正在读取实验…" /> : !detail ? <Empty title={failure ? "实验已删除或无法读取" : "还没有实验结果"}>{failure || "运行因子分析、回测或模型训练后，在这里查看真实结果。"}</Empty> : <><nav className="r-subtabs" aria-label="结果视图">{[["overall", "整体表现"], ...(detail.experiment.kind === "backtest.run" || detail.tables.some(t=>/trade|交易/i.test(t.name)) ? [["replay", "个股交易回放"]] : []), ["tables", "完整数据表"], ["compare", "实验比较"], ["parameters", "参数与输出"]].map(([key, label]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}</nav>{tab === "overall" && <><p>{detail.experiment.summary}</p>{detail.experiment.kind!=="factor.analyze"&&<><SnapshotDifference detail={detail}/>{["model.train","backtest.run","optimize.run"].includes(detail.experiment.kind)&&<PreparationHistory detail={detail}/>}</>}<UnavailableFactors details={detail.details} /></>}
      ...
      {tab!=="replay"&&<div className="r-toolbar"><button onClick={() => exportResult("csv")}>导出 CSV</button><button onClick={() => exportResult("xlsx")}>导出 Excel</button><button onClick={() => exportResult("pdf")}>导出 PDF 报告</button></div>}</>}</section></div></div>;
}
