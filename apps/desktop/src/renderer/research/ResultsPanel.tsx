...
import { errorText, request, useResearch, jobTitle } from "./state";
import { DataTable, Empty, Plot, valueText, fieldLabel, tableLabel } from "./ui";
import { BacktestReplay, BacktestAssumptions } from "./BacktestReplay";
import { PagedExperimentTable, type TablePage } from "./PagedTable";

import { SnapshotDifference, PreparationHistory } from "./ExperimentContext";
import { FactorAnalysis, ModelAnalysis, PortfolioAnalysis } from "./AnalysisViews";
import { useObjectPanel, useWorkspace } from "./workspace";
import { tableValue } from "./ui";
import { object } from "./configuration";

import { useResultExport } from "./readCancellation";
import { PreviousComparison } from './ComparisonSummary';
import { ComparePanel } from './WorkspacePanels';
...
export function ResultsPanel() {
  const s = useResearch(); const panel=useObjectPanel(),w=useWorkspace(); const [failure,setFailure]=useState(""); const id = s.selectedExperiment || s.experiments[0]?.id || "";
  useEffect(() => { if(id && !s.selectedExperiment) s.setSelectedExperiment(id); }, [id, s.selectedExperiment]);
  const [detail, setDetail] = useState<ExperimentDetails | null>(null); const [loading, setLoading] = useState(false);
  const [selected, setSelected] = useState<string[]>([]);
  const [tab, setTabState] = useState(panel?.view ?? "overall");
  const setTab=(view:string)=>{setTabState(view);if(panel)w.updatePanel(panel.id,{view});};
  useEffect(()=>{if(panel?.view)setTabState(panel.view);},[panel?.view]); const [name, setName] = useState("");
  const [replayRow, setReplayRow] = useState<JsonObject | undefined>();
  const loadedIdentity=useRef("");
  useEffect(() => { let active = true; const identity=JSON.stringify([s.project?.id,id]); const changed=loadedIdentity.current!==identity; loadedIdentity.current=identity; if(changed){setDetail(null);setReplayRow(panel?.symbol?{symbol:panel.symbol,date:panel.date??"",tradeId:panel.tradeId??""}:undefined);} setFailure(""); if (!id) return; if(changed)setLoading(true); void request<ExperimentDetails>("experiments.get", { projectId: s.project?.id, experimentId: id }).then(d=>{if(active){setDetail(d);setName(d.experiment.name);}}).catch(e=>{if(active){setFailure(errorText(e));setDetail(null);}}).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [id, s.project?.id, s.revision]);
  const exporter=useResultExport(`${s.project?.id}:${id}`);
  const exportResult = (format: "csv" | "xlsx" | "pdf") => void s.act(async () => {
    if (!detail || exporter.busy) return;
    if (format !== "pdf") {
  ...
  async function rerun(original:boolean){if(!detail||rerunning)return;setRerunning(true);try{await s.act(async()=>{const experiment=detail.experiment;await request("jobs.submit",{spec:{projectId:experiment.projectId,strategyId:experiment.strategyId,kind:experiment.kind,name:`${experiment.name} · ${original?"原输入复现":"最新数据重算"}`,parameters:original?{}:experiment.parameters,...(original?{inputExperimentId:experiment.id}:{})}});await s.refresh();s.setNotice(original?"已提交原输入复现，使用实验固定输入，生成新实验。":"已提交最新数据重算，原实验保持不变。");});}finally{setRerunning(false);}}
  const onRow = (row: JsonObject) => { if(row.symbol ?? row.instrument ?? row.code) {setReplayRow(row);setTab("replay");} };
  return <div className={`r-page r-results-page${tab==="overall"&&detail?.experiment.kind==="factor.analyze"?" r-factor-result-page":""}`}><div className="r-results-layout"><div className="r-result-selector"><label className="r-field"><select aria-label="选择实验结果" value={id} onChange={e=>{s.setSelectedExperiment(e.target.value);setTab("overall");}}>{s.experiments.map(e=><option key={e.id} value={e.id}>{e.starred?"★ ":""}{jobTitle(e.kind,e.name)} · {new Date(e.createdAt).toLocaleDateString("zh-CN")}</option>)}</select></label><details className="r-result-compare-picker"><summary>实验比较{selected.length ? `（${selected.length}）` : ""}</summary><div className="r-compare-choices">{s.experiments.map(e=><label key={e.id}><input type="checkbox" checked={selected.includes(e.id)} onChange={()=>setSelected(ids=>ids.includes(e.id)?ids.filter(x=>x!==e.id):[...ids,e.id])}/><span>{jobTitle(e.kind,e.name)}</span></label>)}</div><button disabled={selected.length<2} onClick={()=>w.open({kind:"compare",title:"实验比较",experimentRefs:selected.flatMap(key=>{const e=s.experiments.find(item=>item.id===key);return e?[{kind:"experiment" as const,projectId:e.projectId,experimentId:e.id,title:e.name}]:[];})})}>比较已选（{selected.length}）</button></details>{detail&&<details className="r-result-more"><summary>更多操作</summary><div className="r-toolbar">{tab==="replay"&&<><button disabled={exporter.busy} onClick={()=>exportResult("csv")}>导出 CSV</button><button disabled={exporter.busy} onClick={()=>exportResult("xlsx")}>导出 Excel</button><button disabled={exporter.busy} onClick={()=>exportResult("pdf")}>导出 PDF 报告</button></>}<input aria-label="实验名称" value={name} onChange={e => setName(e.target.value)} /><button disabled={!name.trim()} onClick={() => void s.act(async () => { await request("experiments.update", { projectId: s.project?.id, experimentId: id, name: name.trim() }); await s.refresh(); })}>重命名</button><button onClick={() => void s.act(async () => { await request("experiments.update", { projectId: s.project?.id, experimentId: id, starred: !detail.experiment.starred }); await s.refresh(); })}>{detail.experiment.starred ? "取消收藏" : "收藏"}</button><button className="r-danger" onClick={() => { if (window.confirm(`删除实验“${detail.experiment.name}”？`)) void s.act(async () => { await request("experiments.delete", { projectId: s.project?.id, experimentId: id }); s.setSelectedExperiment(""); setSelected(ids => ids.filter(x => x !== id)); await s.refresh(); }); }}>删除</button></div></details>}</div><section className="r-result-content">{exporter.busy&&<div className="r-toolbar" role="status"><span>{exporter.saving?"请选择保存位置…":exporter.cancelling?"正在请求中止导出…":"正在生成导出文件…"}</span>{!exporter.saving&&<button disabled={exporter.cancelling} onClick={exporter.cancel}>取消导出</button>}</div>}{loading ? <Empty title="正在读取实验…" /> : !detail ? <Empty title={failure ? "实验已删除或无法读取" : "还没有实验结果"}>{failure || "运行因子分析、回测或模型训练后，在这里查看真实结果。"}</Empty> : <><div className="r-toolbar r-experiment-reproduction"><button disabled={rerunning||detail.experiment.inputSnapshot?.status!=="available"} onClick={()=>void rerun(true)}>原输入复现</button><button disabled={rerunning} onClick={()=>void rerun(false)}>使用最新数据重算</button><span className="r-note">{detail.experiment.inputSnapshot?.status==="available"?"原输入复现使用固定输入；重算使用当前数据。两者均另存实验。":"此实验没有可用的固定输入，不能原输入复现；可用最新数据另存实验。"}</span></div><nav className="r-subtabs" aria-label="结果视图">{[["overall", "整体表现"], ...(detail.experiment.kind === "backtest.run" || detail.tables.some(t=>/trade|交易/i.test(t.name)) ? [["replay", "个股交易回放"]] : []), ["tables", "完整数据表"], ["compare", "实验比较"], ["parameters", "参数与输出"]].map(([key, label]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}</nav>{tab === "overall" && <><PreviousComparison detail={detail}/><p>{detail.experiment.summary}</p>{detail.experiment.kind!=="factor.analyze"&&<><SnapshotDifference detail={detail}/>{["model.train","backtest.run","optimize.run"].includes(detail.experiment.kind)&&<PreparationHistory detail={detail}/>}</>}<UnavailableFactors details={detail.details} /></>}
      {tab === "overall" && (detail.experiment.kind === "factor.analyze" ? <FactorAnalysis key={detail.experiment.id} detail={detail} /> : <><Metrics metrics={detail.experiment.metrics} kind={detail.experiment.kind} />{detail.experiment.kind === "model.train" ? <ModelAnalysis detail={detail}/> : detail.experiment.kind === "backtest.run" ? <><PortfolioAnalysis detail={detail}/><BacktestAssumptions detail={detail}/></> : <><SeriesPlot details={[detail]}/><ExtendedResults detail={detail}/></>}</>)}
      {tab === "tables" && <>{detail.tables.length ? detail.tables.map(t => <PagedExperimentTable key={`${id}-${t.name}`} experimentId={id} table={artifactTableName(detail, t.name)} onRow={onRow} />) : <Empty title="这个实验没有返回表格" />}</>}
      {tab === "replay" && <BacktestReplay key={detail.experiment.id} detail={detail} initialRow={replayRow}/>}
      {tab === "compare" && <ComparePanel key={`${panel?.id}:${id}`} panelId={panel?.id ?? id} refs={panel?.experimentRefs ?? (detail ? [{kind:"experiment",projectId:detail.experiment.projectId,experimentId:detail.experiment.id,title:detail.experiment.name}] : [])}/>}

      {tab === "parameters" && <><SnapshotDifference detail={detail}/><PreparationHistory detail={detail}/><ApplyBest detail={detail} /><h3>输入参数</h3><pre>{JSON.stringify(detail.experiment.parameters, null, 2)}</pre><h3>详细信息</h3><pre>{JSON.stringify(detail.details, null, 2)}</pre><DataTable table={{ name: "输出文件", columns: ["name", "type", "path"], rows: detail.experiment.artifacts.map(a => ({ ...a })) }} /></>}
      {tab!=="replay"&&tab!=="compare"&&<div className="r-toolbar"><button disabled={exporter.busy} onClick={() => exportResult("csv")}>导出 CSV</button><button disabled={exporter.busy} onClick={() => exportResult("xlsx")}>导出 Excel</button><button disabled={exporter.busy} onClick={() => exportResult("pdf")}>导出 PDF 报告</button></div>}</>}</section></div></div>;
}
...
function Metrics({ metrics,kind="" }: { metrics: Record<string, number | null>;kind?:string }) { return <div className="r-metrics">{Object.entries(metrics).map(([key, value]) => <div key={key}><span>{fieldLabel(key,kind)}</span><strong>{value == null ? "—" : Number.isFinite(value) ? tableValue(value,key,kind) : "不可用"}</strong></div>)}</div>; }
...
function ApplyBest({ detail }: { detail: ExperimentDetails }) {
  ...
  return <section><h3>寻优选出的参数</h3><pre>{JSON.stringify(best, null, 2)}</pre><button onClick={() => void s.act(async () => {
    ...
    const settings: JsonObject = { ...s.project!.settings, [target]: config };
    if (Array.isArray(config.factorIds)) settings.selectedFactors = config.factorIds.filter((v): v is string => typeof v === "string");
    if (config.factorProcessing !== undefined) settings.factorProcessing = structuredClone(config.factorProcessing);
    if (Array.isArray(config.customFactors)) {
      const definitions = new Map<string, JsonObject>();
      for (const entry of [...(Array.isArray(settings.customFactors) ? settings.customFactors : []), ...config.customFactors]) {
        const factor = object(entry); if (typeof factor.id === "string") definitions.set(factor.id, factor);
      }
      settings.customFactors = [...definitions.values()];
    }
