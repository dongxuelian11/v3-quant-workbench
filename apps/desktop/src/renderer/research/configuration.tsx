...
import { ResearchPresetToolbar } from "./ResearchPresetToolbar";
import { Field } from "./ui";
import { flushDrafts } from "./workspace";
...
export function customFactors(settings: JsonObject) {
  return Array.isArray(settings.customFactors) ? settings.customFactors.flatMap(f => {
    const c = object(f);
    return typeof c.id === "string" && typeof c.name === "string" && (typeof c.expression === "string" || c.source === "parquet" && typeof c.dataPath === "string") ? [{ ...c, id: c.id, name: c.name }] : [];
  }) : [];
}
...
interface EditorProps { presets?: boolean; value: JsonObject; onChange: (v: JsonObject) => void }
...
export function ValidationEditor({ value, onChange, dates, onDates, presets=true, showDates=true }: EditorProps & { dates: JsonObject; onDates: (v: JsonObject) => void; showDates?:boolean }) {
  return <section>{presets&&<ResearchPresetToolbar category="validation" value={value} onChange={onChange}/>}<ChoiceSetting value={value} onChange={onChange} label="验证方式" name="mode" choices={{ single: "单次时间划分", rolling: "滚动验证" }} />{value.mode === "rolling" && <div className="r-form-grid">{[["trainYears", "训练年数"], ["validMonths", "验证月数"], ["testMonths", "测试月数"], ["stepMonths", "滚动步长（月）"]].map(([name, label]) => <NumberSetting key={name} value={value} onChange={onChange} name={name} label={label} min={1} step={1} />)}</div>}{showDates&&<><p className="r-note">草稿日期可以留空；运行前请明确填写完整六个日期，训练、验证、测试区间依次分离。滚动按自然月对齐交易日。</p><div className="r-form-grid">{dateKeys.map((key, i) => <Field key={key} label={dateLabels[i]}><input type="date" value={String(dates[key] ?? "")} onChange={e => onDates({ ...dates, [key]: e.target.value })} /></Field>)}</div></>}</section>;
}
export function validateDates(dates: JsonObject) {
  const v = dateKeys.map(k => String(dates[k] ?? ""));
  if (v.some(d => !d) || !(v[0] <= v[1] && v[1] < v[2] && v[2] <= v[3] && v[3] < v[4] && v[4] <= v[5])) throw new Error("请填写完整、按时间顺序且互不重叠的训练／验证／测试日期。");
}
export function FactorProcessingEditor({ value: supplied, onChange, presets=true }: Partial<EditorProps> = {}) {
