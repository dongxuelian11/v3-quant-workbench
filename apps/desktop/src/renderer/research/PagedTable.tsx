...
export function PagedExperimentTable({ experimentId, table, onRow, initialSymbol = "", projectId: explicitProjectId }: { projectId?: string; experimentId: string; table: string; initialSymbol?: string; onRow?: (row: JsonObject) => void }) {
  ...
  useEffect(() => {
    ...
    setPage(null);
    void (async () => {
      const result = await request<TablePage>("experiments.table", { projectId: explicitProjectId ?? s.project?.id, experimentId, table, offset, limit, ...Object.fromEntries(Object.entries(filters).filter(([, value]) => value)) });
