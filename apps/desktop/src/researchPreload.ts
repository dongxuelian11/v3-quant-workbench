...
const desktop: ResearchBridge["desktop"] = {
...
const research: ResearchBridge = {
  ...
  chooseDirectory: (options) => ipcRenderer.invoke("research:choose-directory", options),
  chooseFiles: (options) => ipcRenderer.invoke("research:choose-files", options),
  readReportDocument: (reportId) => ipcRenderer.invoke("research:report-document", reportId),
  openExternal: (url) => ipcRenderer.invoke("research:open-external", url),
...
contextBridge.exposeInMainWorld("v3Desktop", desktop);
