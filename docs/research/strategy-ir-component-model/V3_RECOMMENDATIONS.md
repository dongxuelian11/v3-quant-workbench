...
Build canonical Strategy IR now, but treat this research as architecture and implementation input rather than a contract edit. The existing V3 ASL already exposes the right seams—validate draft, compile IR, publish immutable StrategyVersion, worker isolation and handoff. The next phase should make their semantic payload precise. **Disposition: ADOPT**.
...
| P0 | Freeze immutable StrategyVersion over canonical IR, exact bindings, compiler/runtime and code dependency hashes | Extends current `compileStrategyIr` / `publishStrategyVersion` seam | ADOPT |
...
| P0 | Require exact DatasetVersion/DataSnapshotVersion/UniverseVersion and hashes | Aligns with ProjectContextRevision and published version registries | ADOPT |
...
|---|---|---|---|
| `StrategyDraft` stores mode/code/review/validation/handoff | No semantic IR/projection split or exact binding manifest | Define DraftSemanticIR, VisualProjection, CodeProjection and revision fields | ADAPT |
...
| Handoff ID derives from editable draft version | Not enough immutability/provenance | Require published StrategyVersion and exact output/binding references | ADAPT |
...
| ASL publish requires matching IR/validation and immutable StrategyVersion | Good invariant | Add binding/runtime/code/provenance content identity | ADOPT |
...
| StrategyVersion BindingManifest | Exact Dataset/Snapshot/Universe plus compiler/runtime/code hashes | ADOPT |
...
3. StrategyVersion must be immutable and content-identified over IR, exact inputs, compiler/runtime and custom-code dependencies. **Disposition: ADOPT**.
...
9. Slippage, commissions and execution algorithms belong to Backtest/Execution scenarios and must not alter StrategyVersion identity. **Disposition: REJECT**.
...
| Exact data binding creates more StrategyVersions | Prefer explicit reproducibility; optionally distinguish internal DefinitionVersion from user-facing bound StrategyVersion | ADAPT |
