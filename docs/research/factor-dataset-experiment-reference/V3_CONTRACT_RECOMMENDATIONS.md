...
| Data Truth V1区分Snapshot publication、Strict PIT与validation-profile admission | 下游统一继承minimum upstream truth ceiling；`PUBLISHED + STRICT_PIT`不得解释为`FORMAL_ADMITTED` | `ADOPT_INVARIANT` |
...
definition_determinism_profile + compiler_semantics_version
...
`ADOPT_INVARIANT`：FactorDefinitionVersion只回答“怎么算”，不包含某次`DataSnapshotVersion`、`UniverseVersion`、historical membership、knowledge cutoff、exact calendar version或runtime environment/engine fingerprint。这些实际输入变化时，definition ID保持不变。

## 2. `FactorEvaluation` / `FeatureMaterialization`

`ADOPT_INVARIANT`：新增清晰的求值/物化对象，identity至少覆盖：

```text
factor_evaluation_id / feature_materialization_id
factor_definition_version_id
exact data_snapshot_version_id + snapshot content hash
exact universe_version_id + resolved historical membership hash
knowledge_cutoff + exact calendar_version_id
materialization schema/fingerprints + requested time range
environment_profile_id + engine/compiler runtime fingerprint
cache key + output semantic hash + artifact refs
computed truth ceiling + admission evidence
provenance {research_run_id, producer_attempt_id}
```

`ADOPT_INVARIANT`：ResearchRun/Attempt是producer provenance，不应仅因retry换了Attempt就改变相同canonical evaluation的semantic identity；但任何identity input、code或environment semantics变化都必须创建新的Evaluation/Run identity。

`ADOPT_INVARIANT`：Snapshot、UniverseVersion、membership resolution、knowledge cutoff、calendar version或runtime engine变化时，FactorDefinitionVersion ID保持不变；FactorEvaluation/FeatureMaterialization/ResearchRun identity、cache key、output semantic hash与provenance必须改变。Cross-sectional factor也遵守同一规则，不能把exact membership写回definition identity。
...
├─ FactorEvaluation/feature materialization refs and hashes
...
## 13. Provenance closure 与 Result publication
...
## 14. Truth ceiling 继承规则

`ADOPT_INVARIANT`：每个求值、物化、Dataset、Research Run、Model、Prediction、Signal、Backtest与Result对象都必须保存exact upstream truth states与计算后的ceiling：

```text
downstream.truth_state <= minimum(all required upstream truth states)
```

`PUBLISHED`只表示对象生命周期已经发布；Strict PIT PASS只表示在给定decision time/knowledge cutoff下的可见性与修订选择可证明；两者都不等于validation profile `FORMAL_ADMITTED`。若任一required Snapshot/Universe/Dataset是`PRE_ALPHA / NOT_FORMAL`，后续factor proof、leakage proof、Artifact closure或Run success无论多完整，下游最多仍是`PRE_ALPHA / NOT_FORMAL`。

`ADOPT_INVARIANT`：Formal path只有在exact upstream Snapshot validation profile已`FORMAL_ADMITTED`、Universe/PIT/revision/knowledge-cutoff gates满足、Factor dependency/lookahead与Dataset leakage/preprocessing/split proofs PASS、safe Artifact/provenance完整时，才有资格进入下一层Formal admission。每层可以维持或降低truth state，永远不能自行提升。

`REJECT_NOT_V3_FIT`：禁止以Qlib/MLflow success、回测指标、人工标签、完整manifest或Prediction→Signal promotion绕过truth ceiling。Prediction仍不是Signal；独立Signal admission也不能提高其输入truth state。
