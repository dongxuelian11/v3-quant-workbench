...
// The TDX adapter runs in a child process; its older HTTP dependencies must not replace AI dependencies.
runPython(python, ["-m", "pip", "install", "--target", resolve(dirname(python), "tdx-site"), "--upgrade", "-r", resolve(root, "apps/backend/requirements-tdx.txt"), "--cache-dir", resolve(root, ".cache/pip"), "--disable-pip-version-check", "--no-warn-script-location"]);
