...
app.whenReady().then(async () => {
  ...
  if (phase === "restart") {
    ...
    if (restored.lab !== "research" || restored.panelTabs < 3 || restored.layoutContract !== "precision-workbench-v2") throw new Error(`Restart state not restored ${JSON.stringify(restored)}`);
    ...
    if (!interactionEvidence.keyboardTraversal.representativeTraversalPass || !interactionEvidence.dockview.focusable || !interactionEvidence.motion.reducedMotionRulePresent) throw new Error(`Keyboard/motion evidence failed ${JSON.stringify(interactionEvidence)}`);
    await delay(900);
