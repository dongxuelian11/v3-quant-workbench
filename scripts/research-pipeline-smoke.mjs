...
if (!selected) {
  console.error("Core research pipeline smoke requires CPython 3.14.x. Set V3_PYTHON to an explicit interpreter.");
  process.exit(1);
}

console.log(`Using ${selected.version} for the bounded PRE_ALPHA research pipeline smoke.`);
const environment = {
  ...process.env,
...
const result = spawnSync(
  selected.command,
  [...selected.prefix, "-B", "-m", "apps.backend.tests.core_research_pipeline.smoke"],
