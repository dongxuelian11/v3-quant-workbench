...
if (!selected) {
  console.error("Model pipeline smoke requires CPython 3.14.x. Set V3_PYTHON to an explicit interpreter.");
  process.exit(1);
}

console.log(`Using ${selected.version} for the bounded PRE_ALPHA Model pipeline smoke.`);
const environment = {
  ...process.env,
...
const result = spawnSync(
  selected.command,
  [...selected.prefix, "-B", "-m", "apps.backend.tests.model_pipeline_runnability.smoke"],
